...
def validate_http_json_schema_org_draft_07_schema(data):
    ...
    if isinstance(data, dict):
        ...
        if "$comment" in data_keys:
            ...
            if not isinstance(data_comment, (str)):
                raise JsonSchemaException("data.$comment must be string")
        ...
        if "readOnly" in data_keys:
            ...
            if not isinstance(data_readOnly, (bool)):
                raise JsonSchemaException("data.readOnly must be boolean")
        ...
        if "maxLength" in data_keys:
            ...
            validate_http_json_schema_org_draft_07_schema_definitions_nonnegativeinteger(data_maxLength)
        if "minLength" in data_keys:
            data_keys.remove("minLength")
            data_minLength = data["minLength"]
            validate_http_json_schema_org_draft_07_schema_definitions_nonnegativeintegerdefault0(data_minLength)
        ...
        if "additionalItems" in data_keys:
            ...
            validate_http_json_schema_org_draft_07_schema(data_additionalItems)
        if "items" in data_keys:
            ...
            if not data_items_any_of_count:
                try:
                    validate_http_json_schema_org_draft_07_schema(data_items)
                    data_items_any_of_count += 1
                except JsonSchemaException: pass
            if not data_items_any_of_count:
                try:
                    validate_http_json_schema_org_draft_07_schema_definitions_schemaarray(data_items)
                    data_items_any_of_count += 1
                except JsonSchemaException: pass
            if not data_items_any_of_count:
        ...
        if "maxItems" in data_keys:
            ...
            validate_http_json_schema_org_draft_07_schema_definitions_nonnegativeinteger(data_maxItems)
        if "minItems" in data_keys:
            data_keys.remove("minItems")
            data_minItems = data["minItems"]
            validate_http_json_schema_org_draft_07_schema_definitions_nonnegativeintegerdefault0(data_minItems)
        ...
        if "contains" in data_keys:
            ...
            validate_http_json_schema_org_draft_07_schema(data_contains)
        if "maxProperties" in data_keys:
            data_keys.remove("maxProperties")
            data_maxProperties = data["maxProperties"]
            validate_http_json_schema_org_draft_07_schema_definitions_nonnegativeinteger(data_maxProperties)
        if "minProperties" in data_keys:
            data_keys.remove("minProperties")
            data_minProperties = data["minProperties"]
            validate_http_json_schema_org_draft_07_schema_definitions_nonnegativeintegerdefault0(data_minProperties)
        if "required" in data_keys:
            data_keys.remove("required")
            data_required = data["required"]
            validate_http_json_schema_org_draft_07_schema_definitions_stringarray(data_required)
        if "additionalProperties" in data_keys:
            ...
            validate_http_json_schema_org_draft_07_schema(data_additionalProperties)
            ...
            if isinstance(data_definitions, dict):
                ...
                for data_definitions_key in data_definitions_keys:
                    if data_definitions_key not in "dict_keys([])":
                        ...
                        validate_http_json_schema_org_draft_07_schema(data_definitions_value)
            ...
            if isinstance(data_properties, dict):
                ...
                for data_properties_key in data_properties_keys:
                    if data_properties_key not in "dict_keys([])":
                        ...
                        validate_http_json_schema_org_draft_07_schema(data_properties_value)
            ...
            if isinstance(data_patternProperties, dict):
                ...
                for data_patternProperties_key in data_patternProperties_keys:
                    if data_patternProperties_key not in "dict_keys([])":
                        ...
                        validate_http_json_schema_org_draft_07_schema(data_patternProperties_value)
            if isinstance(data_patternProperties, dict):
                if len(data_patternProperties) == 0:
                    pass
                else:
                    for key in data_patternProperties_keys:
                        validate_http_json_schema_org_draft_07_data_patternproperties_property_names(key)
                ...
                for data_dependencies_key in data_dependencies_keys:
                    if data_dependencies_key not in "dict_keys([])":
                        ...
                        if not data_dependencies_value_any_of_count:
                            try:
                                validate_http_json_schema_org_draft_07_schema(data_dependencies_value)
                                data_dependencies_value_any_of_count += 1
                            except JsonSchemaException: pass
                        if not data_dependencies_value_any_of_count:
                            try:
                                validate_http_json_schema_org_draft_07_schema_definitions_stringarray(data_dependencies_value)
                                data_dependencies_value_any_of_count += 1
                            except JsonSchemaException: pass
                        if not data_dependencies_value_any_of_count:
        ...
        if "propertyNames" in data_keys:
            ...
            validate_http_json_schema_org_draft_07_schema(data_propertyNames)
        ...
        if "type" in data_keys:
            ...
            if not data_type_any_of_count:
                try:
                    validate_http_json_schema_org_draft_07_schema_definitions_simpletypes(data_type)
                ...
                try:
                    ...
                    if isinstance(data_type, list):
                        for data_type_x, data_type_item in enumerate(data_type):
                            validate_http_json_schema_org_draft_07_schema_definitions_simpletypes(data_type_item)
                    data_type_any_of_count += 1
                except JsonSchemaException: pass
            if not data_type_any_of_count:
        ...
        if "contentMediaType" in data_keys:
            ...
            data_contentMediaType = data["contentMediaType"]
            if not isinstance(data_contentMediaType, (str)):
                raise JsonSchemaException("data.contentMediaType must be string")
        if "contentEncoding" in data_keys:
            data_keys.remove("contentEncoding")
            data_contentEncoding = data["contentEncoding"]
            if not isinstance(data_contentEncoding, (str)):
                raise JsonSchemaException("data.contentEncoding must be string")
        if "if" in data_keys:
            data_keys.remove("if")
            data_if = data["if"]
            validate_http_json_schema_org_draft_07_schema(data_if)
        if "then" in data_keys:
            data_keys.remove("then")
            data_then = data["then"]
            validate_http_json_schema_org_draft_07_schema(data_then)
        if "else" in data_keys:
            data_keys.remove("else")
            data_else = data["else"]
            validate_http_json_schema_org_draft_07_schema(data_else)
        if "allOf" in data_keys:
            data_keys.remove("allOf")
            data_allOf = data["allOf"]
            validate_http_json_schema_org_draft_07_schema_definitions_schemaarray(data_allOf)
        if "anyOf" in data_keys:
            data_keys.remove("anyOf")
            data_anyOf = data["anyOf"]
            validate_http_json_schema_org_draft_07_schema_definitions_schemaarray(data_anyOf)
        if "oneOf" in data_keys:
            data_keys.remove("oneOf")
            data_oneOf = data["oneOf"]
            validate_http_json_schema_org_draft_07_schema_definitions_schemaarray(data_oneOf)
        if "not" in data_keys:
            data_keys.remove("not")
            data_not = data["not"]
            validate_http_json_schema_org_draft_07_schema(data_not)
    return data

def validate_http_json_schema_org_draft_07_schema_definitions_simpletypes(data):
...
def validate_http_json_schema_org_draft_07_data_patternproperties_property_names(data):
    if isinstance(data, str):
        if not is_valid_regexp(data):
            raise JsonSchemaException("data must be a valid regex")
    return data

def validate_http_json_schema_org_draft_07_schema_definitions_stringarray(data):
...
def validate_http_json_schema_org_draft_07_schema_definitions_schemaarray(data):
    if not isinstance(data, (list)):
        raise JsonSchemaException("data must be array")
    if isinstance(data, list):
        data_len = len(data)
    ...
    if isinstance(data, list):
        for data_x, data_item in enumerate(data):
            validate_http_json_schema_org_draft_07_schema(data_item)
    return data

def validate_http_json_schema_org_draft_07_schema_definitions_nonnegativeintegerdefault0(data):
    validate_http_json_schema_org_draft_07_schema_definitions_nonnegativeinteger(data)
    return data

def validate_http_json_schema_org_draft_07_schema_definitions_nonnegativeinteger(data):
